...
- The `namespaceSelector` for namespaced target resources is now a mandatory field.
  Previous default behavior of syncing to all namespace can be achieved by setting an
...
The API was changed to simplify trust-manager internals (e.g. validation), but also to make room
