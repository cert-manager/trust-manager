...
func Test_BuildBundle(t *testing.T) {
	tests := map[string]struct {
		...
		"if selects no ConfigMap sources, should return NotFoundError": {
			sources: []trustmanagerapi.BundleSource{
				{SourceReference: trustmanagerapi.SourceReference{Name: "configmap", Kind: trustmanagerapi.ConfigMapKind, Selector: &metav1.LabelSelector{MatchLabels: map[string]string{"selects-nothing": "true"}}}, Key: "key"},
			},
			expError:         true,
			expNotFoundError: true,
		},
		"if selects at least one ConfigMap source, return data": {
			sources: []trustmanagerapi.BundleSource{
				{SourceReference: trustmanagerapi.SourceReference{Name: "configmap", Kind: trustmanagerapi.ConfigMapKind, Selector: &metav1.LabelSelector{MatchLabels: map[string]string{"trust-bundle.certs": "includes"}}}, Key: "key"},
				{SourceReference: trustmanagerapi.SourceReference{Name: "configmap", Kind: trustmanagerapi.ConfigMapKind, Selector: &metav1.LabelSelector{MatchLabels: map[string]string{"selects-nothing": "true"}}}, Key: "key"},
			},
			objects: []runtime.Object{&corev1.ConfigMap{
		...
		"if selects at least one ConfigMap source including all keys, return data": {
			sources: []trustmanagerapi.BundleSource{
				{SourceReference: trustmanagerapi.SourceReference{Kind: trustmanagerapi.ConfigMapKind, Selector: &metav1.LabelSelector{MatchLabels: map[string]string{"trust-bundle.certs": "includes"}}}, Key: "*"},
			},
			objects: []runtime.Object{
				&corev1.ConfigMap{
					ObjectMeta: metav1.ObjectMeta{Name: "configmap", Labels: map[string]string{"trust-bundle.certs": "includes"}},
		...
		"if selects at least one Secret source including all keys, return data": {
			sources: []trustmanagerapi.BundleSource{
				{SourceReference: trustmanagerapi.SourceReference{Kind: trustmanagerapi.SecretKind, Selector: &metav1.LabelSelector{MatchLabels: map[string]string{"trust-bundle.certs": "includes"}}}, Key: "*"},
			},
			objects: []runtime.Object{
				&corev1.Secret{
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			spec := trustmanagerapi.BundleSpec{
				Sources:           tt.sources,
				IncludeDefaultCAs: tt.includeDefaultCAs,
