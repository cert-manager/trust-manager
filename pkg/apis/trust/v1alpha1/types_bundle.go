...
// +kubebuilder:validation:XValidation:rule="has(oldSelf.configMap) == has(self.configMap)", message="target configMap removal is not allowed"
// +kubebuilder:validation:XValidation:rule="has(oldSelf.secret) == has(self.secret)", message="target secret removal is not allowed"
