...
DESTINATION_FILE=${1:-}
BIN_VALIDATE_TRUST_PACKAGE=${2:-}

function print_usage() {
	echo "usage: $0 <destination file> <validate-trust-package binary>"
}
...
if [[ -z $BIN_VALIDATE_TRUST_PACKAGE ]]; then
	print_usage
	echo "validate-trust-package binary must be specified"
	exit 1
fi
...
$CTR run --rm --mount type=bind,source="$TMP_DIR",target=/workdir $DEBIAN_IMAGE /bin/bash /workdir/run.sh
